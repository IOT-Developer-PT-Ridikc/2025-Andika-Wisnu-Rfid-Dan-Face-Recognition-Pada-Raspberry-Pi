...
default_config = {
"kamera" : 0,
"timer_duration" : 5,  # Timer countdown dalam detik
...
def encoding_wajah():
    ...
    for nama in os.listdir(folder_wajah):
        ...
        if os.path.isdir(sub_folder):
            for b in tqdm(os.listdir(sub_folder)):
                ...
                if os.path.isfile(path_image):
                    ...
                    # Dapatkan encoding wajah menggunakan face_recognition
        ...
        # Tambahkan encoding ke data_semua jika ada
        if poto:
...
def deteksi():
    global timer_start, timer_deteksi, elapsed_time, sekali_kirim, last_detected_name, timer_duration, nama, id, data_nama
    ...
    while True:
        ...
        print( time.time() - timer_deteksi)
        if time.time() - timer_deteksi < 20 :
            print("DETEKSI AKTIF ... ", round(time.time() - timer_deteksi,2))
